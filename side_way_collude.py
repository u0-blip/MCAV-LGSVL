...
state.transform.position = spawns[0].position + 2.5 * right

npc_start = state.transform
...
for i in range(20):
    ...
    angle = npc_start.rotation
    # Raycast the points onto the ground because BorregasAve is not flat
    hit = sim.raycast(npc_start.position + pz * forward,
...
# for i in range(200):
#     sim.run(0.2)
#     npc.state.velocity = a.state.velocity
